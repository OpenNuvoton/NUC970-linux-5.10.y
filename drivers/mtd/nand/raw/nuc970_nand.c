...
#define NUC970_DRV_VERSION "20230701"
...
static int nuc970_ooblayout_ecc(struct mtd_info *mtd, int section,
...
static int nuc970_ooblayout_free(struct mtd_info *mtd, int section,
				  struct mtd_oob_region *oobregion)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct nand_ecc_ctrl *ecc = &chip->ecc;
...
static const struct mtd_ooblayout_ops nuc970_ooblayout_ops = {
	.ecc = nuc970_ooblayout_ecc,
	.free = nuc970_ooblayout_free,
};
...
static struct nand_ecclayout_user nuc970_nand_oob;
 ...
 */
static void nuc970_nand_hwecc_init(struct nuc970_nand_info *nand)
{
 ...
 */
static int nuc970_nand_correct_data(struct nand_chip *chip, u_char *dat, u_char *read_ecc, u_char *calc_ecc)
{
	return 0;
}
 ...
 */
void nuc970_nand_enable_hwecc(struct nand_chip *chip, int mode)
{
 ...
 */
static void nuc970_nand_dmac_init( struct nuc970_nand_info *nand )
{
	// DMAC enable
 ...
 */
static unsigned char nuc970_nand_read_byte(struct nand_chip *chip)
{
	unsigned char ret;
...
static void nuc970_nand_read_buf(struct nand_chip *chip, unsigned char *buf, int len)
{
	int i;
...
static void nuc970_nand_write_buf(struct nand_chip *chip, const unsigned char *buf, int len)
{
	int i;
...
static inline int nuc970_nand_dma_transfer(struct nand_chip *chip, const u_char *addr, unsigned int len, int is_write)
{
	struct nuc970_nand_info *nand = nand_get_controller_data(chip);
	dma_addr_t dma_addr;
	int ret;
...
static void nuc970_read_buf_dma(struct nand_chip *chip, u_char *buf, int len)
{
	struct mtd_info *mtd = nand_to_mtd(chip);

	if ( len == mtd->writesize ) /* start transfer in DMA mode */
		nuc970_nand_dma_transfer (chip, buf, len, 0x0);
	else {
		nuc970_nand_read_buf(chip, buf, len);
...
static void nuc970_write_buf_dma(struct nand_chip *chip, const u_char *buf, int len)
{
	struct mtd_info *mtd = nand_to_mtd(chip);

	if ( len == mtd->writesize ) /* start transfer in DMA mode */
		nuc970_nand_dma_transfer(chip, (u_char *)buf, len, 0x1);
	else
	{
#ifdef NUC970_NAND_DEBUG
		int i;
...
#endif
		nuc970_nand_write_buf(chip, buf, len);
	}
}


static int nuc970_nand_devready(struct nand_chip *chip)
{
...
static int nuc970_waitfunc(struct nand_chip *chip)
{
...
static void nuc970_nand_command(struct nand_chip *chip, unsigned int command, int column, int page_addr)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
...
/* select chip */
static void nuc970_nand_select_chip(struct nand_chip *chip, int cs)
{
...
static int nuc970_nand_calculate_ecc(struct nand_chip *chip, const u_char *dat, u_char *ecc_code)
{
	return 0;
}

static int nuc970_nand_write_page_hwecc(struct nand_chip *chip, const uint8_t *buf, int oob_required, int page)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	...
	nuc970_nand_command(chip, NAND_CMD_SEQIN, 0, page);
	nuc970_nand_dma_transfer(chip, buf, mtd->writesize , 0x1);
	nuc970_nand_command(chip, NAND_CMD_PAGEPROG, -1, -1);
	nuc970_waitfunc(chip);
...
static int nuc970_nand_read_page_hwecc_oob_first(struct nand_chip *chip, uint8_t *buf, int oob_required, int page)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	uint8_t *p = buf;
	char * ptr=REG_SMRA0;
	...
	nuc970_nand_command(chip, NAND_CMD_READOOB, 0, page);
	nuc970_nand_read_buf(chip, chip->oob_poi, mtd->oobsize);
	...
	else
	{
		...
		nuc970_nand_command(chip, NAND_CMD_READ0, 0, page);
		nuc970_nand_dma_transfer(chip, p, mtd->writesize, 0x0);
...
static void nuc970_layout_oob_table ( struct nand_ecclayout_user* pNandOOBTbl, int oobsize , int eccbytes )
{
...
static int nuc970_nand_read_oob_hwecc(struct nand_chip *chip, int page)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	char * ptr=REG_SMRA0;
...
static irqreturn_t nuc970_nand_irq(int irq, struct nuc970_nand_info *nand)
{
...
static int nuc970_nand_attach_chip(struct nand_chip *chip)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	struct nuc970_nand_info *host = nand_get_controller_data(chip);
	unsigned int reg;

	/*============================================*/
	if ((mtd->writesize == 2048) && (chip->ecc.strength == 8) && (mtd->oobsize == 128))
		mtd->oobsize = 64;
	/*============================================*/
	...
	if (mtd->writesize == 2048) {
		...
		host->m_ePageSize = ePageSize_2048;
	}
	else if (mtd->writesize == 4096) {
		writel(reg | 0x20000, REG_SMCSR);
		host->m_ePageSize = ePageSize_4096;
	}
	else if (mtd->writesize == 8192) {
		writel(reg | 0x30000, REG_SMCSR);
		host->m_ePageSize = ePageSize_8192;
	} else
		host->m_ePageSize = ePageSize_512;

	if (chip->ecc.strength == 4)
		host->eBCHAlgo = eBCH_T4; /* T4 */
	...
	else if (chip->ecc.strength <= 15)
		host->eBCHAlgo = eBCH_T15; /* T15 */
	...
	nuc970_layout_oob_table(&nuc970_nand_oob, mtd->oobsize, g_i32ParityNum[host->m_ePageSize][host->eBCHAlgo]);

	host->m_i32SMRASize = mtd->oobsize;
	chip->ecc.steps = mtd->writesize / chip->ecc.size;
	chip->ecc.bytes = nuc970_nand_oob.eccbytes / chip->ecc.steps;
	chip->ecc.total = nuc970_nand_oob.eccbytes;
	mtd_set_ooblayout(mtd, &nuc970_ooblayout_ops);

	pr_info("attach: page %d, SMRA size %d, %d\n", mtd->writesize, mtd->oobsize, nuc970_nand_oob.eccbytes);

	/* add mtd-id. The string should same as uboot definition */
	mtd->name = "nand0";

	nuc970_nand_hwecc_init(host);
...
static const struct nand_controller_ops nuc970_nand_controller_ops = {
	.attach_chip = nuc970_nand_attach_chip,
};
...
static int nuc970_nand_probe(struct platform_device *pdev)
{
	...
	nand_controller_init(&nuc970_nand->controller);

	nuc970_nand->pdev = pdev;
	nuc970_nand->dev = &pdev->dev;
	chip = &nuc970_nand->chip;
	mtd = nand_to_mtd(chip);
	nand_set_controller_data(chip, nuc970_nand);
	...
	chip->legacy.cmdfunc     = nuc970_nand_command;
	chip->legacy.waitfunc    = nuc970_waitfunc;
	chip->legacy.read_byte   = nuc970_nand_read_byte;
	chip->legacy.select_chip = nuc970_nand_select_chip;
	chip->legacy.read_buf    = nuc970_read_buf_dma;
	chip->legacy.write_buf   = nuc970_write_buf_dma;
	chip->legacy.chip_delay  = 25; /* us */

	// Check NAND device NBUSY0 pin
	chip->legacy.dev_ready   = nuc970_nand_devready;
	/* set up nand options */
	...
	nuc970_nand->controller.ops = &nuc970_nand_controller_ops;

	nuc970_nand->irq = platform_get_irq(pdev, 0);
	if (nuc970_nand->irq < 0) {
		dev_err(&pdev->dev, "failed to get platform irq\n");
		retval = -EINVAL;
		goto fail;
	}

	if (request_irq(nuc970_nand->irq, (irq_handler_t)&nuc970_nand_irq,
			IRQF_TRIGGER_HIGH, "nuc970-nand", nuc970_nand)) {
	...
	if (mtd_device_register(mtd, nuc970_nand->parts, nuc970_nand->nr_parts))
		goto fail;
...
fail:
	...
	nand_cleanup(chip);
	devm_kfree(&pdev->dev, nuc970_nand);
	return retval;
}
...
static int nuc970_nand_resume(struct platform_device *pdev)
{
	...
	clk_enable(nuc970_nand->clk);
	nuc970_nand_hwecc_init(nuc970_nand);
	writel(0x1, REG_NFECR); /* un-lock write protect */
	nuc970_nand_dmac_init(nuc970_nand);
