...
static int report_touch(struct nuc970_adc *priv);
static void detect_touch(struct nuc970_adc *priv);
static void detect_pendown(struct nuc970_adc *priv);


static irqreturn_t nuc970_adc_interrupt(int irq, void *private)
{
	struct nuc970_adc *priv = (struct nuc970_adc *)private;
	u32 isr, wkisr;
...
static void nuc970adc_ts_tasklet(struct nuc970_adc *priv)
{
...
{
	struct nuc970_adc *priv = container_of(hrtimer,
						struct nuc970_adc,
						trigger_hrt);
	unsigned long flags;
...
static void detect_wakeup(struct nuc970_adc *priv)
{
	unsigned long flags;
...
static void stop_detect(struct nuc970_adc *priv)
{
	unsigned long flags;
...
static int nuc970_ts_open(struct input_dev *dev)
{
	struct nuc970_adc *priv = input_get_drvdata(dev);
...
static void nuc970_ts_close(struct input_dev *dev)
{
	struct nuc970_adc *priv = input_get_drvdata(dev);
...
static int nuc970_ts_register(struct platform_device *pdev)
{
	struct nuc970_adc *priv = platform_get_drvdata(pdev);
	struct input_dev *ts_dev;
	int ret;
	...
	if (!ts_dev)
		return -ENOMEM;
	ts_dev->name = "NUC970 Touch Screen";
	ts_dev->phys = "nuc970ts/event0";
	ts_dev->id.bustype = BUS_HOST;
	ts_dev->dev.parent = &pdev->dev;
	ts_dev->open = nuc970_ts_open;
	ts_dev->close = nuc970_ts_close;
...
static void nuc970_ts_unregister(struct platform_device *pdev)
{
	struct nuc970_adc *priv = platform_get_drvdata(pdev);
...
static void normal_convert(struct nuc970_adc *priv, int ch)
{
	unsigned long flags;
...
static int nuc970_adc_read_raw(struct iio_dev *idev,
			     struct iio_chan_spec const *chan,
			     int *val, int *val2, long mask)
{
	struct nuc970_adc *priv = iio_device_get_drvdata(idev);
...
static int nuc970_iio_register(struct platform_device *pdev)
{
	int ret, bit, idx = 0;
	struct iio_dev *indio_dev;
	struct nuc970_adc *priv = platform_get_drvdata(pdev);
...
static int nuc970_iio_unregister(struct platform_device *pdev)
{
	struct nuc970_adc *priv = platform_get_drvdata(pdev);
...
static int nuc970_adc_probe_dt(struct nuc970_adc *priv,
...
static int nuc970_adc_probe(struct platform_device *pdev)
{
	...
	struct nuc970_adc *priv;
	int ret;

	dev_info(&pdev->dev, "Nuvoton NUC970 Touch ADC Driver\n");
	priv = devm_kzalloc(dev, sizeof(*priv), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;

	ret = nuc970_adc_probe_dt(priv, pdev);
	if (ret) {
	...
	ret = devm_request_irq(&pdev->dev, priv->irq, nuc970_adc_interrupt,
				0, dev_name(dev), priv);
	if (ret) {
	...
	if (priv->enable_ts) {
		ret = nuc970_ts_register(pdev);
		if (ret)
			goto error_disable_clk;
	}

	if (priv->enable_iio) {
		ret = nuc970_iio_register(pdev);
		if (ret)
...
static SIMPLE_DEV_PM_OPS(nuc970_adc_pm_ops,
		nuc970_adc_suspend, nuc970_adc_resume);

static const struct of_device_id nuc970_adc_match[] = {
...
static struct platform_driver nuc970_adc_driver = {
	...
	.driver = {
		.name = "nuc970-adc",
		.of_match_table = nuc970_adc_match,
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Nuvoton NUC970 ADC Driver");
