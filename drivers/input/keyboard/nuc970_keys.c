/* linux/driver/input/nuc970_keys.c
 *
...
static struct input_dev *nuc970_keys_input_dev;
...
static void scan_key(struct timer_list *t)
{
	...
	for (j = 0; j < col_cnt; j++) {
		if(is_keypad_mode) {
			for (i = 0; i < col_cnt; i++) {
				...
				if(i == j) {
					writel(readl(REG_GPIOA_DATAOUT + group*0x40) & ~( 0x1 << num ),
						 		 REG_GPIOA_DATAOUT + group*0x40); // write low
				} else {
					writel(readl(REG_GPIOA_DATAOUT + group*0x40) | ( 0x1 << num ),
								 REG_GPIOA_DATAOUT + group*0x40); // write high
				}
			}
			udelay(100);
		}
		for (i = 0; i < row_cnt; i++) {
			...
			index = !(readl(REG_GPIOA_DATAIN + group*0x40) & (0x1 << num));
	...
	for (i = 0; i < key_cnt; i++) {
		if ((new_key ^ old_key) & (0x1 << i)) {
			input_event(nuc970_keys_input_dev, EV_KEY, key_code[i], (new_key & (0x1 << i)) == (0x1 << i));
			input_sync(nuc970_keys_input_dev);
		} else if (new_key & (0x1 << i)) {
			// pressing
			input_event(nuc970_keys_input_dev, EV_KEY, key_code[i], 2);
			input_sync(nuc970_keys_input_dev);
		}
	}
...
static irqreturn_t nuc970_kpd_irq(int irq, void *dev_id)
{
	...
	// clear ISR
	writel(readl(REG_GPIOA_ISR + group*0x40) & (0x1 << num), REG_GPIOA_ISR + group*0x40);
...
int nuc970_kpd_open(struct input_dev *input)
{
	u32 i;
	int error = 0;
	...
	for (i = 0; i < row_cnt; i++) {
		// clear isr status
		writel(0x1 << (gpio_row[i] % GPIO_OFFSET), REG_GPIOA_ISR + (gpio_row[i] / GPIO_OFFSET)*0x40);
		error =  request_irq(IRQ_GPIO_START + gpio_row[i], nuc970_kpd_irq, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_NO_SUSPEND, "Key", NULL);
		if (error) {
	...
	if(is_keypad_mode) {
		...
		for (i = 0; i < col_cnt; i++) {
			// clear isr status
			writel(0x1 << (gpio_col[i] % GPIO_OFFSET), REG_GPIOA_ISR + (gpio_col[i] / GPIO_OFFSET)*0x40);
...
static int nuc970_keys_probe(struct platform_device *pdev)
{
	int i, err, group, num;
	u32	gpio_matrix[2] = {0};
	...
	for (i = 0; i < row_cnt; i++) {
		...
		group = gpio_row[i] / GPIO_OFFSET;
		num = gpio_row[i] % GPIO_OFFSET;
		writel(readl(REG_GPIOA_DIR + group*0x40) & ~( 0x1 << num ),
					 REG_GPIOA_DIR + group*0x40); // input
		writel(readl(REG_GPIOA_PUEN + group*0x40) | ( 0x1 << num ),
					 REG_GPIOA_PUEN + group*0x40); // pull-up
	...
	if (is_keypad_mode) {
		for (i = 0; i < col_cnt; i++) {
			...
			num = gpio_col[i] % GPIO_OFFSET;
			writel(readl(REG_GPIOA_DIR + group*0x40) | ( 0x1 << num ),
						 REG_GPIOA_DIR + group*0x40); // output
			writel(readl(REG_GPIOA_DATAOUT + group*0x40) & ~( 0x1 << num ),
						 REG_GPIOA_DATAOUT + group*0x40); // write low
			writel(readl(REG_GPIOA_DBEN + group*0x40) | ( 0x1 << num ),
					 	 REG_GPIOA_DBEN + group*0x40); // debounce enable
	...
	if (!(nuc970_keys_input_dev = input_allocate_device())) {
		dev_err(&pdev->dev, "Failed to allocate memory.\n");
		err = -ENOMEM;
		goto fail;
	}

	nuc970_keys_input_dev->name = "nuc970-keys";
	nuc970_keys_input_dev->phys = "input/event1";
	nuc970_keys_input_dev->id.bustype = BUS_HOST;
	nuc970_keys_input_dev->id.vendor  = 0x0005;
	nuc970_keys_input_dev->id.product = 0x0001;
	nuc970_keys_input_dev->id.version = 0x0100;

	nuc970_keys_input_dev->open    = nuc970_kpd_open;
	nuc970_keys_input_dev->close   = nuc970_kpd_close;

	nuc970_keys_input_dev->evbit[0] = BIT(EV_KEY) | BIT(EV_SYN) |  BIT(EV_REP);

	for (i = 0; i < key_cnt; i++)
		set_bit(key_code[i], nuc970_keys_input_dev->keybit);

	err = input_register_device(nuc970_keys_input_dev);
	if (err) {
		input_free_device(nuc970_keys_input_dev);
		return err;
	}

	// must set after input device register!!!
	nuc970_keys_input_dev->rep[REP_DELAY] = 250;
	nuc970_keys_input_dev->rep[REP_PERIOD] = 33;
...
static int nuc970_keys_remove(struct platform_device *pdev)
{
	u32 i;
	int err;
	struct clk *clk;
...
#define nuc970_keys_suspend   NULL
#define nuc970_keys_resume	  NULL
...
static struct platform_driver nuc970_keys_driver = {
	.probe		= nuc970_keys_probe,
	.remove		= nuc970_keys_remove,
	.suspend	= nuc970_keys_suspend,
	.resume		= nuc970_keys_resume,
	.driver		= {
		.name	= "nuc970-keys",
		.owner	= THIS_MODULE,
...
module_platform_driver(nuc970_keys_driver);

MODULE_AUTHOR("nuvoton");
MODULE_DESCRIPTION("nuc970 keys driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:nuc970-keys");
