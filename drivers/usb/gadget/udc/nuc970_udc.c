/*
 * linux/drivers/usb/gadget/udc/nuc970_udc.c
 *
...
static void udc_isr_reset(struct nuc970_udc *udc);
static void udc_isr_dma(struct nuc970_udc *udc);
static void udc_isr_ctrl_pkt(struct nuc970_udc *udc);
static void udc_isr_update_dev(struct nuc970_udc *udc);
static void nuc970_udc_enable(struct nuc970_udc *udc);
static void nuc970_udc_disable(struct nuc970_udc *udc);
...
static void nuke (struct nuc970_udc *udc, struct nuc970_ep *ep, int status)
{
 ...
 *  write_packet
 */
static inline int write_packet(struct nuc970_ep *ep, struct nuc970_request *req)
{
	struct nuc970_udc *udc = ep->dev;
	unsigned total, len;
	u8  *buf;
	...
	else	/* in-transfer */
	{
		...
		else
		{
			...
			__raw_writel((__raw_readl(udc->base + REG_USBD_DMACTL)&0xe0) | 0x10 | ep->ep_num,
...
void paser_usb_irq(struct nuc970_udc *udc, int irq)
{
...
void paser_irq_cep(struct nuc970_udc *udc, u32 irq)
{
	struct nuc970_ep *ep = &udc->ep[0];
	struct nuc970_request *req;
	int is_last = 1;
	unsigned int volatile timeout;
...
void paser_irq_nep(struct nuc970_ep *ep, u32 irq)
{
	struct nuc970_udc *udc = ep->dev;
	struct nuc970_request *req;
	unsigned int volatile timeout;
	...
	{
		if (ep->index == 5)
			__raw_writel(USBD_EPINTSTS_RXPKIF|USBD_EPINTSTS_SHORTRXIF|0x800, udc->base + REG_USBD_EPA_EPINTSTS + 0x28*(ep->index-1));
		else
...
{
	struct nuc970_udc *udc;
	u32 volatile IrqStL, IrqSt;
	int j;

	udc = (struct nuc970_udc *)(_dev);
...
static int nuc970_queue(struct usb_ep *_ep, struct usb_request *_req, gfp_t gfp_flags)
{
	...
	{
		pr_err("nuc970_queue: invalid args\n");
 ...
 * the endpoint.
 */
static int nuc970_fifo_status(struct usb_ep *_ep)
{
	struct nuc970_ep *ep;
	struct nuc970_udc *udc;
	u32 bytes = 0;
...
static void nuc970_fifo_flush(struct usb_ep *_ep)
{
	struct nuc970_ep *ep;
	struct nuc970_udc *udc;
	unsigned long flags;
...
static const struct usb_ep_ops nuc970_ep_ops =
{
	...
	.fifo_status	= nuc970_fifo_status,
	.fifo_flush		= nuc970_fifo_flush,
};
...
{
	struct nuc970_udc *udc = to_nuc970_udc(_gadget);
	int tmp;
...
static int nuc970_udc_stop(struct usb_gadget *g);
...
void udc_isr_update_dev(struct nuc970_udc *udc)
{
	...
	switch (pcrq->bRequest)
	{
		...
		case USB_REQ_SET_FEATURE:
			...
			nuc970_set_halt(&udc->ep[pcrq->wIndex & 0x7f].ep, 1);
			break;

		case USB_REQ_CLEAR_FEATURE:
			nuc970_set_halt(&udc->ep[pcrq->wIndex & 0x7f].ep, 0);
			break;
...
static void USB_Init(struct nuc970_udc *udc)
{
	int i, j;
...
static int nuc970_udc_probe(struct platform_device *pdev)
{
	...
	error = request_irq(udc->irq, nuc970_udc_irq, 0, gadget_name, udc);
	if (error != 0)
	{
	...
	pr_info("nuc970 usb device probe ok\n");
