...
__u8 g_au8QTable1[64] = { 0x06, 0x06, 0x06, 0x08, 0x07, 0x08, 0x10, 0x09,
			  ...
			  0x21, 0x21, 0x21, 0x21, 0x21, 0x21, 0x21, 0x21,
			  0x21, 0x21, 0x21, 0x21, 0x21, 0x21, 0x21, 0x21 };
__u8 g_au8QTable2[64] = { 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03,
...
__u8 g_au8QTableUser0[64] = { 0x06, 0x04, 0x04, 0x05, 0x04, 0x04, 0x06, 0x05,
			  ...
			  0x20, 0x21, 0x23, 0x23, 0x23, 0x15, 0x1A, 0x26,
			  0x29, 0x26, 0x22, 0x28, 0x1F, 0x22, 0x23, 0x21 };
__u8 g_au8QTableUser1[64] = { 0x06, 0x06, 0x06, 0x08, 0x07, 0x08, 0x10, 0x09,
			  0x09, 0x10, 0x21, 0x16, 0x13, 0x16, 0x21, 0x21,
			  0x21, 0x21, 0x21, 0x21, 0x21, 0x21, 0x21, 0x21,
			  0x21, 0x21, 0x21, 0x21, 0x21, 0x21, 0x21, 0x21,
			  0x21, 0x21, 0x21, 0x21, 0x21, 0x21, 0x21, 0x21,
...
)
{
	if (u8Mode == DRVJPEG_ENC_UPSCALE_MODE)
	{
		...
		_DRVJPEG_ENC_ENABLE_UPSCALING();        
		   ...
		   u8Mode == DRVJPEG_DEC_PLANAR_DOWNSCALE_MODE ||
		   u8Mode == DRVJPEG_ENC_PLANAR_DOWNSCALE_MODE)	{
	...
	if (u8Mode == DRVJPEG_DEC_PLANAR_DOWNSCALE_MODE ||
	    u8Mode == DRVJPEG_ENC_PLANAR_DOWNSCALE_MODE)
		writel((readl(REG_JPSCALD) & ~(PSCALX_F | PSCALY_F)) |
		       ((u16FactorW & 0x1F) << 8) | (u16FactorH & 0x1F), REG_JPSCALD);
	else {
...
)
{
	...
	for (i = 0; i < 64; i = i + 4) {
		...
		if(!u32TimeOut)    
			return -EINVAL;
		u32value = puQTable0[i] | (puQTable0[i+1]<<8) |
			  (puQTable0[i+2]<<16) | (puQTable0[i+3]<<24);
	...
	for (i = 0; i < 64; i=i+4) {
		...
		u32value = puQTable1[i] | (puQTable1[i+1]<<8) |
			   (puQTable1[i+2]<<16) | (puQTable1[i+3]<<24);
...
static int nuc970_jpegcodec_probe(struct platform_device *pdev)
{
	...
	priv->vaddr = (__u32)dma_alloc_coherent(&pdev->dev, CONFIG_JPEG_CODEC_BUFFER_SIZE,
	...
	result = video_register_device(&priv->jdev, VFL_TYPE_SUBDEV, -1);
	if (result) {
	...
	ret = request_irq(IRQ_JPEG, (irq_handler_t)jpegirq_handler,
			  IRQF_SHARED, "nuc970-jpeg", priv);
...
static int __exit nuc970_jpegcodec_remove(struct platform_device *pdev)
{
	...
	dma_free_coherent(&pdev->dev, CONFIG_JPEG_CODEC_BUFFER_SIZE,
			  (void *)priv->vaddr, priv->paddr);
