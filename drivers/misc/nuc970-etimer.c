...
static irqreturn_t nuc970_etimer_interrupt(int irq, void *dev_id)
{
	...
	if (__raw_readl(REG_ETMR_ISR(ch)) & 0x10 ) {
		/* clear system wake up source flag */
		__raw_writel((0x100000 << ch), REG_WKUPSSR);

		/* clear Timer Wake-up Status */
		__raw_writel(0x10, REG_ETMR_ISR(ch));
	}
	...
	if (__raw_readl(REG_ETMR_ISR(ch)) & 0x1) {
		...
		__raw_writel(__raw_readl(REG_ETMR_ISR(ch)) & 0x1, REG_ETMR_ISR(ch));
		t->update = 1;
	}
	...
	if (__raw_readl(REG_ETMR_ISR(ch)) & 0x2) {
		...
		__raw_writel(__raw_readl(REG_ETMR_ISR(ch)) & 0x2, REG_ETMR_ISR(ch));
	}
...
static ssize_t etimer_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	spin_lock_irqsave(&t->lock, flag);
	if (t->mode != ETIMER_OPMODE_TRIGGER_COUNTING && t->mode != ETIMER_OPMODE_FREE_COUNTING &&
