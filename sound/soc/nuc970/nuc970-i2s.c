...
static int nuc970_i2s_hw_params(struct snd_pcm_substream *substream,
				 struct snd_pcm_hw_params *params,
				 struct snd_soc_dai *dai)
{
	struct nuc970_audio *nuc970_audio = nuc970_i2s_data;
	unsigned long val = AUDIO_READ(nuc970_audio->mmio + ACTL_CON);
...
static int nuc970_i2s_set_fmt(struct snd_soc_dai *cpu_dai, unsigned int fmt)
{
	struct nuc970_audio *nuc970_audio = nuc970_i2s_data;
...
static int nuc970_i2s_set_sysclk(struct snd_soc_dai *cpu_dai, int clk_id, unsigned int freq, int dir)
{
	unsigned int val;
	struct nuc970_audio *nuc970_audio = nuc970_i2s_data;
...
static int nuc970_i2s_trigger(struct snd_pcm_substream *substream, int cmd, struct snd_soc_dai *dai)
{
	struct nuc970_audio *nuc970_audio = nuc970_i2s_data;
	int ret = 0;
...
static struct snd_soc_dai_ops nuc970_i2s_dai_ops = {
	...
	.hw_params  = nuc970_i2s_hw_params,
...
static int nuc970_i2s_drvprobe(struct platform_device *pdev)
{
	...
	if (!request_mem_region(nuc970_audio->res->start, resource_size(nuc970_audio->res), pdev->name)) {
		ret = -EBUSY;
		goto out0;
	}

	nuc970_audio->mmio = ioremap(nuc970_audio->res->start, resource_size(nuc970_audio->res));
	...
	ret = snd_soc_register_component(&pdev->dev, &nuc970_i2s_component, &nuc970_i2s_dai, 1);
	if (ret)
		goto out3;
...
out1:
	release_mem_region(nuc970_audio->res->start, resource_size(nuc970_audio->res));
out0:
	kfree(nuc970_audio);
...
static int nuc970_i2s_drvremove(struct platform_device *pdev)
{
	...
	release_mem_region(nuc970_i2s_data->res->start, resource_size(nuc970_i2s_data->res));
