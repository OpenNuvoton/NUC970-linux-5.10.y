...
static int nuc970_audio_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
...
SND_SOC_DAILINK_DEFS(hifi,
                     DAILINK_COMP_ARRAY(COMP_CPU("b0009000.i2s")),
                     DAILINK_COMP_ARRAY(COMP_CODEC("nau8822.0-001a", "nau8822-hifi")),
                     DAILINK_COMP_ARRAY(COMP_PLATFORM("b0009000.i2s")));
...
static int nuc970_audio_probe(struct platform_device *pdev)
{
	...
	if (np) {
		nuc970evb_i2s_dai.cpus->of_node = of_parse_phandle(np, "i2s-controller", 0);
		if (!nuc970evb_i2s_dai.cpus->of_node) {
